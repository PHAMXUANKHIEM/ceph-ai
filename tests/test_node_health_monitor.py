...
def test_check_node_resources_does_not_flag_below_threshold(monkeypatch):
    ...
    monkeypatch.setattr(nhm.node_resource_forecast, "fetch_latest_metrics", lambda cluster, host: _metrics(cpu=50.0, mem=50.0))
...
def test_check_node_resources_flags_after_consecutive_high_scans(monkeypatch):
    monkeypatch.setattr(nhm, "configured_nodes", lambda: [{"host": "10.0.0.5", "roles": ["MON"]}])
    monkeypatch.setattr(nhm.node_resource_forecast, "fetch_latest_metrics", lambda cluster, host: _metrics(cpu=95.0, mem=40.0))
...
def test_check_node_resources_flags_on_ram_alone(monkeypatch):
    ...
    monkeypatch.setattr(nhm.node_resource_forecast, "fetch_latest_metrics", lambda cluster, host: _metrics(cpu=20.0, mem=95.0))
...
def test_check_node_resources_resets_streak_when_a_scan_is_normal(monkeypatch):
    ...
    monkeypatch.setattr(nhm.node_resource_forecast, "fetch_latest_metrics", lambda cluster, host: high)
    for _ in range(nhm.CONSECUTIVE_SCANS_REQUIRED - 1):
        nhm.check_node_resources()

    monkeypatch.setattr(nhm.node_resource_forecast, "fetch_latest_metrics", lambda cluster, host: normal)
    assert nhm.check_node_resources() == {}
...
def test_check_node_resources_skips_a_host_that_fails_to_collect(monkeypatch):
    ...
    def fake_collect(host):
        if host == "broken":
            raise nhm.node_resource_forecast.NodeResourceLokiError("loki failed")
...
def test_create_or_resolve_creates_incident_and_investigate_manually_action(isolated_db):
    ...
    with db_module.SessionLocal() as session:
        ...
        assert evidence["source"] == "loki_node_resource_metrics"
