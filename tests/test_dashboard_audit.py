...
def test_dashboard_lists_all_audit_entries_with_no_filter(dashboard_client):
...
def test_dashboard_audit_filters_by_incident_id(dashboard_client):
    ...
    response = dashboard_client.get("/", params={"incident_id": "inc-a"})

    assert response.status_code == 200
    assert "safe_action_executed" in response.text
    assert "risky_action_approved" not in response.text
    # Filter value is echoed back into the form so it survives a reload.
    assert 'value="inc-a"' in response.text


def test_dashboard_audit_filters_by_time_range(dashboard_client):
    ...
    response = dashboard_client.get("/", params={"since": since})
...
def test_dashboard_audit_ignores_unparseable_filter_instead_of_500(dashboard_client):
    _seed_incident_and_entry("inc-x", "safe_action_executed", datetime.utcnow())
    _login(dashboard_client)

    response = dashboard_client.get("/", params={"since": "not-a-date"})
