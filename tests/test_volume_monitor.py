...
@pytest.mark.parametrize(
    "error",
    [
        "All MON nodes failed: 10.20.1.39: TimeoutError",
        "All MON nodes failed: 10.20.1.39: 10.20.1.39: command exited 124: "
        "rbd: waiting for initial image stats",
    ],
)
def test_check_volumes_stops_after_all_mon_nodes_time_out(monkeypatch, error):
    monkeypatch.setattr(vm.ceph_client, "configured_rbd_pools", lambda: ["slow", "not-queried"])
    queried = []

    def fake_iostat(pool):
        queried.append(pool)
        raise CephQueryError(error)
