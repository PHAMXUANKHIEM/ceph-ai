...
def test_get_telegram_alerts_shows_compact_overview_for_admin(dashboard_client):
    ...
    assert 'href="/telegram-alerts/backup"' in response.text
    ...
    assert 'name="bot_token"' not in response.text
    assert 'name="chat_id"' not in response.text


def test_get_telegram_channel_detail_shows_configuration_and_history(dashboard_client):
    _login(dashboard_client)

    response = dashboard_client.get("/telegram-alerts/backup")

    assert response.status_code == 200
    assert "Cảnh báo Backup" in response.text
    assert 'href="/telegram-alerts">← Tổng quan</a>' in response.text
    assert 'action="/telegram-alerts/backup"' in response.text
    assert 'action="/telegram-alerts/backup/test"' in response.text
    assert 'name="bot_token"' in response.text
    assert 'name="chat_id"' in response.text
    assert "telegram-history-details" in response.text
    assert "TRUST MODEL" in response.text


def test_get_telegram_channel_detail_rejects_unknown_channel(dashboard_client):
    _login(dashboard_client)
