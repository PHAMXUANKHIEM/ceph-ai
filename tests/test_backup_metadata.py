...
def fakes(monkeypatch):
    backend = FakeBackend()
    # worker/backup/metadata.py resolves targets via
    # worker/backup/cluster_scope.py::resolve_targets() now (multi-tenant
    # remediation Phase 3 consolidation) — patch there, not on `metadata`.
    monkeypatch.setattr(cluster_scope, "get_backend", lambda slot, settings, immutable_enabled=False: backend)
    monkeypatch.setattr(cluster_scope, "backup_targets_from_policy", lambda: DEFAULT_POLICY_TARGETS)
    # ceph_mon_nodes lives on the shared Settings singleton — patching it
    # here is visible from cluster_scope.py's own `configured_nodes(None)`
    # call too (same object, not a per-module copy).
    monkeypatch.setattr(settings, "ceph_mon_nodes", "10.20.1.112,10.20.1.95,10.20.1.21", raising=False)
