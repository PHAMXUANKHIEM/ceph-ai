...
- [ ] Rate limit learning job.
- [ ] Giới hạn batch size và Loki query duration.
- [ ] Timeout, bounded retry và circuit breaker.
- [ ] Retention cho raw sample, forecast run và audit log.
...
- [ ] Chọn một cluster/nhóm node làm canary.
- [ ] Chạy candidate shadow qua một chu kỳ forecast đầy đủ.
- [ ] So sánh MAE/SMAPE, precision/recall, false-positive rate, alert volume, early detection time và DATA_QUALITY rate.
- [ ] Kiểm tra restart worker/watcher không mất state.
- [ ] Kiểm tra migration downgrade/backup.
- [ ] Operator xác nhận trước khi candidate thành active.
...
Bước tiếp theo: Bước 6.4 — safeguards cho learning job, batch size, timeout/retry/circuit breaker và retention/backup.
