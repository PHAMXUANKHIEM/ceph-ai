...
- [~] **3.5 Kiểm thử** — đã có test pool contention, cluster isolation, stale mapping/host, host disk signal, bounded correlation, hot-resource ranking, recovery collector/pgmap parsing, network peer-relative evidence, missing-node isolation và benchmark false-positive; còn thiếu workload recovery/mất node/network end-to-end với collector thật.
