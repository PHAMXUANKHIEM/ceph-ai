...
- [~] **4.2 Bucket access diagnosis** — thêm read-only `GET /api/object-storage/buckets/{bucket}/diagnosis`: phân tích access log RGW + bucket stats + RGW topology, phân biệt S3 auth 401, policy/permission 403, quota reached, 429 rate-limit, 5xx backend/service và CephX/radosgw-admin failure; không tự nới policy/quota hay tạo Action. Còn thiếu read-only DNS/TLS probe và error-message evidence chi tiết từ RGW log.
...
|---|---|---|---|---|---|
| 2026-09-19 | Pha 4.2 — bucket access diagnosis | Một phần | Thêm `watcher/rgw_bucket_diagnosis.py` và API `GET /api/object-storage/buckets/{bucket}/diagnosis`: phân tích HTTP access outcomes, quota stats, endpoint evidence và admin-query status; phân loại auth/policy/quota/rate-limit/backend/not-found, fail-closed khi log rỗng và trả `recommendation_mode=ADVISORY`, `action_id=null`. Chưa probe DNS/TLS và chưa trích xuất error-message chi tiết từ RGW daemon log. | `pytest tests/test_rgw_bucket_diagnosis.py tests/test_rgw_evidence.py tests/test_dashboard_object_storage.py tests/test_object_storage_cache.py` (64/64 pass) + compileall | Chờ commit |
