...
- [~] **1.1 Time-series pipeline**
  - 2026-09-23: thêm collector `rbd du` read-only theo pool cấu hình, opt-in bằng `CEPH_AI_CAPACITY_RBD_ENRICHMENT_ENABLED=true`, giới hạn 4 pool/chu kỳ và 5.000 row/pool; lưu image head, snapshot, provisioned và physical pool usage riêng theo cluster. Chưa có per-volume attribution, replica/EC/failure-domain reserve và chưa kiểm tra live Ceph.
...
- [~] **1.2 Forecast engine** — đã có bản forecast deterministic cho cluster/pool/OSD với xu hướng tuyến tính, weekly residual correction khi đủ lịch sử, spike guard, confidence interval và ngày chạm threshold trong horizon; nguồn volume/snapshot/thin provisioning mới có observation cấp pool opt-in, chưa được đưa vào engine và chưa có mô hình replica/EC đầy đủ.
...
- [~] **1.3 Risk explanation** — UI/API đã nêu method, confidence interval, spike warning, growth direction và giới hạn raw-capacity/replica/EC; còn thiếu attribution theo volume/snapshot/workload và tách overhead replica/EC từ collector thật.
  - UI/API hiện hiển thị bằng chứng RBD cấp pool, thin ratio và tăng trưởng volume/snapshot *quan sát được*; không dùng chúng để tuyên bố forecast đã được hiệu chỉnh theo workload/replica/EC.
...
- [~] **2.4 Recommendation only** — chuẩn hóa insight thành recommendation contract `ADVISORY`: có `read_only=true`, `action_id=null`, expected saving, impact, evidence timestamp và TTL 15 phút; áp dụng cho inventory stale/snapshot, snapshot-clone dependency và backup protection gap. Không tạo Action, không thực thi retain/trash/resize; còn thiếu UI tổng hợp expected saving/impact và TTL theo từng recommendation.
...
- [~] **2.5 Kiểm thử** — đã test attached volume, thiếu lịch sử, zero-I/O stale, snapshot protection, snapshot policy, clone parent/child, partial evidence, bounded route, protection gap (missing/stale/failed backup, restore drill) và cluster/tenant isolation giữa default và secondary cluster; còn thiếu dữ liệu clone/backup thực tế trên nhiều Ceph release.
...
- [~] **3.4 Read-only report** — đã có `/performance-rca` và `/api/performance-rca`, hiển thị hypothesis, evidence chain, freshness, evidence gaps, hot resources, next checks theo từng hypothesis và ranked read-only options; không tạo Action hay thay đổi PG/CRUSH/replica/EC.
  - Sinh báo cáo nguyên nhân, bằng chứng, bước kiểm tra và phương án xếp hạng.
  - Không tự đổi PG/CRUSH/replica/EC trong pha này.
- [~] **3.5 Kiểm thử** — đã có test pool contention, cluster isolation, stale mapping/host, host disk signal, bounded correlation, hot-resource ranking, public recovery collector/pgmap parsing, network peer-relative evidence, missing-node isolation và benchmark false-positive; còn thiếu workload recovery/mất node/network end-to-end trên cụm thật.
...
- [~] **4.1 RGW evidence collector** — thêm read-only `GET /api/object-storage/rgw-evidence` và `watcher/rgw_evidence.py`: thu thập daemon/endpoint/frontend, realm/zonegroup/zone, sync status và placement-pool capacity dependency; cache theo cluster với stale-if-error tối đa 15 phút, mọi section fail-closed. Còn thiếu kiểm chứng trực tiếp trên nhiều Ceph release và adapter legacy systemd đầy đủ.
...
- [~] **4.2 Bucket access diagnosis** — thêm read-only `GET /api/object-storage/buckets/{bucket}/diagnosis`: phân tích access log RGW + bucket stats + RGW topology, phân biệt S3 auth 401, policy/permission 403, quota reached, 429 rate-limit, 5xx backend/service, DNS/TCP/TLS endpoint và CephX/radosgw-admin failure; đọc bounded RGW daemon error messages, không tự nới policy/quota hay tạo Action. Còn thiếu kiểm chứng probe trên nhiều Ceph release và error taxonomy đầy đủ hơn.
...
> - [x] L5 adapter Loki production-ready: đã xác nhận /ready HTTP 200, labels có cluster/daemon_type/host và query_range trả dữ liệu thật cho CS-LAB ngày 2026-09-21.
...
- [~] **8.1 Remediation state machine**
  - Thêm canonical state contract không cần migration: transition hợp lệ, expiry, cancellation, distributed lock và worker-restart recovery fail-closed. Reconciler của autonomous SAFE action đã gọi recovery contract khi lease mất/hết hạn; chưa nối toàn bộ executor/DB lifecycle vào contract.
...
- [~] **9.1 Ba chế độ vận hành**
  - Thêm contract ADVISORY/APPROVAL_REQUIRED/LIMITED_AUTOPILOT; 2026-09-23: `CEPH_AI_AUTOPILOT_MODE` được cưỡng chế tại ranh giới dispatch SAFE action (ADVISORY/APPROVAL_REQUIRED luôn chuyển sang duyệt, giá trị sai fail-closed). Cờ global/cluster cũ vẫn là kill switch.
  - 2026-09-23: bổ sung `CEPH_AI_AUTOPILOT_CLUSTER_MODES` dạng JSON `{\"<cluster_id>\":\"ADVISORY|APPROVAL_REQUIRED|LIMITED_AUTOPILOT\"}`. Khi biến này được cấu hình, cluster không có khóa tương ứng và JSON lỗi đều rơi về `APPROVAL_REQUIRED`; không thể dùng mode global để vô tình cấp quyền cho cluster mới. Đây là lớp cấu hình runtime tạm thời, vẫn cần persisted setting/migration và UI operator để coi là hoàn thành.
...
|---|---|---|---|---|---|
| 2026-09-23 | Pha 1.1/1.3, 8.1 và 9.1 — RBD capacity evidence, lease recovery, runtime mode | Một phần | Thêm migration `m20260923rbdcapacity`, collector `rbd du` opt-in/read-only và bảng evidence volume/snapshot/thin cấp pool; unknown/stale có nhãn, không biến thành zero hay forecast mới. Reconciler SAFE action dùng state-machine recovery khi lease mất. `CEPH_AI_AUTOPILOT_MODE` có thể siết mode tại final SAFE dispatch, không thể vượt global/cluster kill switch; Single Full giữ nguyên. Chưa bật collector production, chưa có forecast alert lifecycle, full executor wiring, per-cluster persisted mode hoặc rollback production. | `pytest` 6 nhóm router/autopilot/state/capacity: **157 passed**; `alembic heads`: 1 head; SQLite fresh upgrade lên head thành công; `git diff --check` sạch; chưa kiểm PostgreSQL/live Ceph | Chưa commit |
...
| 2026-09-21 | L5 — Loki adapter readiness | Hoàn thành | Xác nhận adapter Loki có query_range bounded, UTC nanoseconds, label validation, retry/circuit breaker, partial-result contract và check_reachable; live smoke test trên 10.3.53.25:3100 trả /ready HTTP 200, labels hợp lệ và query CS-LAB có 1 stream/1 entry. | Live smoke test đạt; vẫn cần theo dõi retention/label drift định kỳ | Chờ commit |
...
| 2026-09-19 | Pha 3.5 — live recovery và network evidence | Một phần | Performance RCA đọc `ceph -s`/`pgmap` để nhận biết recovery/backfill/degraded/misplaced đang active hoặc idle; bổ sung network peer-relative evidence từ host metrics, ghi rõ đây chưa phải network saturation tuyệt đối vì chưa có interface capacity baseline. Mọi query fail-closed và không tạo Action. | `pytest tests/test_performance_rca.py tests/test_performance_rca_monitor.py tests/test_performance_simulation.py tests/test_performance_rca_ui.py` (24/24 pass) + compileall | Chờ commit |
| 2026-09-19 | Pha 3.5 — fail-closed performance RCA tests | Một phần | Bổ sung kiểm thử cho recovery/backfill/slow-ops và network evidence còn thiếu, mapping node bị mất, cùng benchmark latency bình thường nhưng có OSD outlier chưa scope. Báo cáo nêu rõ evidence gap và không nâng false-positive thành hot volume; chưa có collector recovery/network thật để test end-to-end. | `pytest tests/test_performance_rca.py tests/test_performance_rca_monitor.py tests/test_performance_simulation.py tests/test_performance_rca_ui.py` (21/21 pass) + `git diff --check` | Chờ commit |
| 2026-09-19 | Pha 3.4 — ranked read-only report guidance | Một phần | Thêm investigation steps có cấu trúc theo từng hypothesis và danh sách ranked options cho volume cần điều tra. UI hiển thị next checks, confidence và lý do xếp hạng; mọi mục đều `read_only=true`, `action_id=null`, không tạo Action và không chạy lệnh Ceph. | `pytest tests/test_performance_rca.py tests/test_performance_rca_monitor.py tests/test_performance_simulation.py tests/test_performance_rca_ui.py` (18/18 pass) + `git diff --check` | Chờ commit |
| 2026-09-19 | Pha 3.3 — duration và replica/EC simulation | Một phần | Bổ sung duration estimate có điều kiện từ recovery throughput evidence, failure-domain risk theo topology input và coding-width/storage-overhead cho replica/EC. Thiếu throughput hoặc topology thì trả `unknown`, không đoán. | `pytest tests/test_performance_simulation.py` (6/6 pass) | Chờ commit |
...
| 2026-09-19 | Pha 1 — capacity forecast quality/safety slice | Một phần | Bổ sung forecast method (linear/seasonal/spike-guarded), confidence interval, dự báo cuối horizon, risk explanation và rolling backtest; cập nhật Dashboard để hiển thị các trường này. 2026-09-23: thêm API read-only `/api/capacity-forecast/alerts` và `alert_lifecycle` trong forecast response, phản ánh NORMAL/OPEN/RESOLVED và trạng thái retry notification. Chưa đánh dấu hoàn thành vì collector vẫn chưa có đầy đủ volume/snapshot/thin provisioning và replica/EC attribution; phần alert UI/lifecycle end-to-end và live evidence còn phải nghiệm thu. | `pytest tests/test_capacity_forecast.py tests/test_rbd_logical_forecast.py` (đạt) + ruff + `git diff --check` | Chờ commit |
