...
Trạng thái: canary production đang ở `SHADOW_ONLY`; chưa promote model mới sang
`ACTIVE`, chưa mở rộng scope và chưa cho candidate ảnh hưởng notification hoặc
remediation.

Đánh giá lại tại `ddc4de7`: AI/ML đã có feature pipeline, model scope,
offline evaluator và các safety boundary; online learning thực tế vẫn chủ yếu
là shadow. Policy ADWIN hiện đã có bounded window/state và fail-closed reset;
active promotion vẫn bị khóa và chưa được coi là production capability. Các boundary
`alibi_detect_boundary`, `bandit_sandbox`, `mlflow_registry_adapter`,
`controlled_action_contract`, `delayed_feedback_evaluator`,
`remediation_state_machine`, `rollback_planner`, `time_series_pipeline` chưa
được coi là production capability nếu chỉ được gọi từ test/evaluator.
...
- [x] Sửa cửa sổ dữ liệu ADWIN trong `watcher/node_resource_forecast.py`: truy vấn
  `evaluated_at DESC`, giới hạn 512 bản ghi mới nhất rồi đảo lại theo thứ tự
  thời gian trước khi replay vào detector.
- [x] Thêm regression test có hơn 512 forecast runs, chứng minh detector nhìn
  thấy drift ở phần dữ liệu mới nhất thay vì 512 dòng cũ nhất.
- [x] Persist ADWIN state theo `cluster + host + metric + detector/model version`
  và khôi phục sau restart; không khởi tạo rồi replay toàn bộ cửa sổ ở mỗi lần
  forecast nếu state còn hợp lệ.
- [x] Có schema version, JSON-safe bounded state và fail-closed reset khi state hỏng,
  sai scope hoặc không tương thích.
- [x] Residual drift giảm confidence và trả trạng thái `DRIFT`; MAE drift đặt
  `promotion_blocked=true`; metric drift trả `METRIC_DRIFT` để không nhầm data
  shift với lỗi model.
- [x] Có hysteresis recovery (`3` mẫu sạch liên tiếp) và warm-up sau drift
  (`5` mẫu); drift không được biến mất chỉ vì một mẫu sạch.
- [x] Có replay read-only so sánh ADWIN với detector cũ 3-sigma, giới hạn tối đa
  512 mẫu và không được ghi model state hoặc trigger promotion.
...
- [ ] Đổi tên Candidate D thành `robust isolation score`/`robust deviation
  score` nếu vẫn dùng z-score/RMS; không gọi là Isolation Forest khi chưa có
  thuật toán isolation thực.
- [ ] Chỉ triển khai Isolation Forest thật sau khi chốt feature contract,
  resource budget, time-series split và benchmark drift injection; giữ ngoài
  production scope cho tới khi có evidence.
...
- [x] So sánh shadow với active bằng paired input window, target timestamp,
  quality status và evidence hash; không dùng prediction đơn lẻ làm evidence.
- [x] Kiểm tra regression theo từng `cluster + host + metric + horizon`, gồm
  stale/partial quality, unpaired data và drift evidence.
- [ ] Thực hiện canary/soak trước active promotion và tự rollback khi metric
  quality/resource vượt ngưỡng đã duyệt.
...
### Phase 6 — Paired evaluation và promotion

#### Bước 6.1 — Paired evaluation `[x]`

- [x] Ghép champion/challenger theo cùng input window, target timestamp và
  quality; unpaired, stale, drift hoặc actual thiếu đều fail-closed.
- [x] Tính MAE, RMSE, SMAPE, bias, p95 absolute error, interval coverage,
  recall và detection delay cho từng model.
- [x] Evidence có scope `cluster + host + metric + horizon`, version, paired
  count, input/target window, quality ratio, drift count, TTL và SHA-256 hash.

#### Bước 6.2 — Promotion gate `[x]`

- [x] Gate configurable cho minimum samples, evaluation streak, data-quality,
  interval coverage, recall, delay và regression MAE/RMSE/SMAPE/p95.
- [x] Gate đánh giá độc lập theo từng host/metric/horizon và chặn candidate có
  drift hoặc evidence hết hạn.
- [x] Có `ForecastModelRegistry` với trạng thái `SHADOW/CANDIDATE/ACTIVE/
  RETIRED`; promotion mặc định vẫn tắt bằng `forecast_promotion_enabled=false`.

#### Bước 6.3 — Approval, audit và rollback `[x]`

- [x] Operator approval gắn cứng scope, version, evidence hash và expiry;
  approval sai scope hoặc hết hạn bị từ chối.
- [x] Promotion/rollback dùng append-only `ForecastPromotionAudit` với các
  event request, blocked, approved, promoted, health failed và rolled back.
- [x] Sau promotion có health verification; nếu unhealthy thì rollback về
  `previous_version`, nếu không có last-known-good thì fail-closed.
- [x] Test đầy đủ paired metrics, gate/streak, approval expiry, registry
  promotion, health failure và rollback.

#### Bước 6.4 — Production wiring `[~]`

- [~] Service và schema đã có; cần nối worker consumer/API operator hiện tại
  vào `evaluate_paired()`/`promotion_gate()` và chỉ bật active sau production
  soak 24–72 giờ. Không tự động promote trong execution wave hiện tại.

### Phase 7 — Dashboard và explainability

#### Bước 7.1 — Online learning status
...
### Phase 8 — Benchmark và nghiệm thu

#### Bước 8.1 — Offline benchmark `[~]`
...
- [ ] Bổ sung time-series split không shuffle, drift injection, delayed-label
  evaluation và nhiều horizon; tách rõ offline evidence với production
  acceptance.
- [ ] Đánh giá Candidate D đúng tên/đúng thuật toán; không ghi Isolation Forest
  nếu implementation chỉ là robust z-score/RMS.
...
#### Bước 8.2 — Production acceptance
...
### Review follow-up — phân biệt shadow learning với active promotion

- [x] Ghi rõ trạng thái hiện tại: consumer đang cố ý gọi `target="shadow"`;
...
- [ ] Rà soát toàn bộ call site của `shared.learning_runtime.evaluate()`.
  `watcher/node_resource_forecast.py` phải truyền đủ `cluster_id`, `host` và
  `metric`; khi canary scope bật, stream đúng scope được phép quan sát còn
  stream thiếu hoặc sai host/metric phải bị chặn fail-closed.
- [ ] Bổ sung regression test riêng cho node forecast với canary scope bật,
  tránh tình trạng dashboard đã truyền đúng scope nhưng Watcher vẫn bỏ qua
  `host`/`metric`.
- [ ] Gắn runtime caller và acceptance evidence cho `alibi_detect_boundary`,
  `bandit_sandbox`, `mlflow_registry_adapter`, `controlled_action_contract`,
  `delayed_feedback_evaluator`, `remediation_state_machine`, `rollback_planner`
  và `time_series_pipeline`; module chỉ được import trong test/evaluator phải
  giữ nhãn `EXPERIMENTAL`.

**Tiêu chí đóng review:** có bằng chứng phân biệt rõ `shadow`, `candidate` và
`active`; không có active update ngoài promotion đã duyệt; runtime gate được
kiểm tra đúng theo `cluster + host + metric` ở tất cả caller.
...
self-learning production có kiểm soát. CI, full-tree quality gate, deploy
workflow và supply-chain reproducibility được giữ ở
`Plan/production-readiness-plan.md` và xử lý sau.
...
- [ ] Bảo đảm candidate/active không được ảnh hưởng notification hoặc
  remediation nếu chưa qua promotion boundary.

### Work package C — Champion–challenger và nhiều horizon

- [ ] Chốt deterministic ensemble làm champion và River làm challenger; không
  promote chỉ vì candidate có prediction hợp lệ.
- [ ] Bổ sung rolling backtest theo từng stream và nhiều horizon phù hợp với
  metric; ghi MAE/SMAPE/bias, interval coverage, false-positive rate, lead time,
  sample count và data-quality status.
- [ ] Dùng paired outcome cùng cửa sổ để so sánh champion/challenger; thiếu
  actual, stale hoặc drift thì kết quả là `INSUFFICIENT_EVIDENCE`.
- [ ] Không đưa Candidate D/deep learning vào production wave trước khi có
  dataset Ceph đã ẩn danh, label đủ và resource budget được nghiệm thu.

### Work package D — Drift, rollback và production acceptance

- [ ] Nối drift detector vào promotion gate; drift hoặc quality regression phải
  chặn promotion và giữ candidate ở shadow.
- [ ] Định nghĩa rollback trigger, RTO mục tiêu, last-known-good champion và
  audit event; kiểm thử sau restart và khi model registry/control DB lỗi.
- [ ] Chạy observation 24–72 giờ trên duy nhất
  `CS-LAB / 10.20.1.153 / cpu`: không mất state, không soft-lockup/poll timeout/
  DB saturation mới, có verified outcome thật và rollback drill đạt thời gian.
- [ ] Chỉ sau khi các mục trên đạt mới xin operator approval; chưa mở rộng
  canary và chưa bật `ACTIVE` trong execution wave này.

### Tiêu chí đóng execution wave

- Có bằng chứng rõ ràng cho ba boundary `shadow`, `candidate`, `active`.
- Mọi caller đều bị scope theo `cluster + host + metric` và có regression test.
- Promotion/rollback có approval, registry state, audit và evidence outcome.
- Canary production vẫn không gửi notification/remediation từ candidate.
- CI/supply-chain chưa đóng thì release decision vẫn là **NO-GO**.
...
- [x] Bước 4.1 — ADWIN window mới nhất, persistent detector state, fail-closed
  reset, residual/MAE policy, hysteresis/warm-up và regression >512 runs đã có.
...
- [x] Phase 6 — paired evaluation, complete metric gate, scoped approval,
  append-only audit và health-triggered rollback service đã có test evidence.
- [~] Bước 6.4 — production wiring vào worker/API, canary/soak và production
  rollback acceptance còn mở; active promotion vẫn bị khóa.
- [~] Bước 5.3 — canary guard, lifecycle evidence, acceptance API/UI và
  resource-cost telemetry read-only đã có; chưa đủ evidence cho active promotion.
- [x] Bước 7.1 — dashboard online-learning status, quality gate, drift, latency và verified feedback read-only.
- [x] Bước 7.2 — forecast detail gồm actual/predicted, interval, model/version, confidence/consensus và MAE/SMAPE.
- [x] Bước 7.3 — operator controls đã viết/test, deploy production và verify hành vi trên live canary.
...
Bước tiếp theo sẽ làm theo thứ tự: **(1) sửa caller scope `cluster + host + metric`,
(2) nối worker/API với paired evaluation và promotion service, (3) chạy E2E
shadow-to-active/rollback, (4) chạy production acceptance 24–72 giờ. CI và
supply-chain để wave sau.
Trong suốt execution wave vẫn giữ River model/PyOD candidate ngoài active
production, chưa promote `ACTIVE` và chưa mở rộng scope.**

### Nhật ký thực hiện ADWIN policy — 2026-09-21

- `shared/adwin_policy.py`: policy ADWIN dùng chung, phân biệt metric drift,
  residual drift và MAE drift; confidence multiplier, promotion gate,
  hysteresis, warm-up và replay comparison.
- `NodeResourceDriftState` + migration `f0a1b2c3d4e5`: lưu state theo đúng
  scope, detector version và JSON-safe window; state hỏng fail-closed.
- `watcher/node_resource_forecast.py`: lấy đúng 512 run mới nhất theo
  `evaluated_at DESC`, đảo thứ tự thời gian, chỉ feed run mới sau cursor và
  lưu drift metadata trên forecast run.
- Test acceptance: `18 passed` cho ADWIN policy và node forecast, gồm residual
  drift, MAE promotion block, metric/model separation, hysteresis/warm-up,
  state round-trip, replay bound và regression hơn 512 run.

### Nhật ký thực hiện Phase 6 — 2026-09-22

- `shared/forecast_promotion.py`: paired evaluator và adapter ghép persisted
  runs theo input window/target timestamp; tính MAE, RMSE, SMAPE, bias, p95,
  interval coverage, recall và delay.
- Gate theo từng `cluster + host + metric + horizon`, có minimum samples,
  quality ratio, evaluation streak, drift/expiry và regression thresholds.
- `ForecastModelRegistry`, `ForecastEvaluationEvidence`, approval có scope /
  version / evidence hash / expiry và `ForecastPromotionAudit` append-only.
- Health verification tự rollback về `previous_version`; nếu thiếu champion tốt
  cuối cùng thì fail-closed.
- Migration `f1a2b3c4d5e6_add_forecast_promotion_phase6.py`; test Phase 6 đạt
  `6 passed`, migration suite đạt `9 passed`. Production wiring và active
  promotion vẫn bị khóa cho tới khi soak 24–72 giờ.
