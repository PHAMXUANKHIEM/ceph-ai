...
- [x] **2.5 Delete và recycle policy**
  ...
  - Đã có move-to-trash, restore-by-ID và purge-all qua Worker; chặn watcher/
    ...
    snapshot tối đa 500 Trash ID, bắt buộc approval. TTL mặc định 7 ngày
    (`RBD_TRASH_RETENTION_DAYS`); item chưa hết hạn vẫn restore được nhưng
    cả delete đơn lẻ và purge-all đều fail-closed.
...
- [~] **6.1 Health/dependency view** từ volume tới pool, PG, OSD và failure
  ...
  - Đã có `/api/volumes/{pool}/dependency-health` và panel Block Storage
    Overview đọc health, PG state, acting OSD, host/failure domain; thiếu PG/OSD
...
- [~] **6.3 Watcher/lock và stale attachment remediation**
  - Volume Detail đã có remediation posture read-only cho `healthy`,
    `stale_attachment`, `orphan`, `mismatch` và `unknown`; stale age không được
...
- [~] **6.4 Data integrity workflow**
  ...
  - Còn live acceptance trên PG degraded/inconsistent thật, đối chiếu checksum
    ...
    có thể đóng mục này.
- [~] **6.5 Pool lifecycle**
  - Đã có inventory read-only `/api/volumes/{pool}/pool-lifecycle` hiển thị
    application tag/RBD capability, PG/PGP và autoscaler mode, quota, số volume
    phụ thuộc, dependency health và operation guard.
  - Pool còn volume bị đánh dấu `HAS_DEPENDENCIES`, dependency critical bị
    `BLOCKED`; chưa có đường create/configure/delete trực tiếp và mọi mutation
    tương lai phải qua Worker + approval + reconciliation.
...
| 2026-09-24 | BS-07.4 / BS-02.4 `rbd_copy_volume` contract sync | Một phần | Đồng bộ action contract: `rbd_copy_volume` chỉ thuộc Dashboard Worker, giữ phân loại `RISKY`, bị loại khỏi Chat/Incident `management_action_ids`; contract công bố target `volume`, params snapshot/destination/size, capability, approval, idempotency, expiry, preflight và post-check. API `/api/v1/block-storage/contract` trả schema này; typed validator fail-closed với pool/image/snapshot/Cinder/size. | `tests/test_action_contract.py tests/test_policy_gate.py tests/test_block_storage_contract.py` cùng command/reconciliation coverage đạt; `git diff --check` đạt. | Còn nối typed gateway trực tiếp vào Dashboard Worker lease nếu muốn dùng chung boundary; hiện action vẫn qua proposal/approval và Worker command/reconciliation riêng. |
