...
| 2026-09-21 | RT-09.3 MON/Cephadm circuit breaker | Health query có circuit breaker theo MON endpoint/credential/mode và `cephadm shell` theo host/credential/command label, cooldown + probe; khi lỗi liên tiếp không tiếp tục tạo SSH/container load, success reset state; diagnostics chỉ trả counter bounded | `config/settings.py`, `watcher/ceph_client.py`, `dashboard/routes/system_health.py`, `tests/test_ceph_client.py`, `tests/test_ceph_debug.py` — **135 passed** |
