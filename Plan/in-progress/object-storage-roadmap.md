...
- [x] **4.2 Xem object metadata, tags, version và retention state**.
  - Object Detail dùng HEAD S3 read-only với owner/endpoint đã validate và
    credential tạm luôn được thu hồi; trả size, content type, ETag, modified,
    storage class, version ID và user metadata. Tags, retention và legal hold
...
- [x] **4.4 Delete/restore version** với confirmation, audit và policy check.
  ...
  - Delete bị fail-closed nếu Object Lock không xác định được, retention còn
    hiệu lực hoặc Legal Hold đang bật; không có bypass governance/compliance.
  - Restore version thường dùng `CopyObject` để tạo current version mới;
    restore delete marker chỉ xóa marker đã chọn để lộ version trước đó.
  - Mọi mutation tạo audit `target_type=object`, không chứa S3 secret, và
...
- [~] **6.3 Read-only diagnostic**: evidence snapshot và multisite diagnosis
  ...
  dẫn kiểm tra tiếp theo; không tự sửa topology. Cần bổ sung runbook chi tiết
  cho từng finding trước khi đóng.
