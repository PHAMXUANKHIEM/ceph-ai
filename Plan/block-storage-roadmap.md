...
| 2026-08-17 | Block Storage IA | Hoàn thành code | Hợp nhất Overview, Volumes & Performance, Trash & Restore vào một nhóm sidebar; Overview có bốn entry point và image link về đúng pool/cluster. Giữ URL cũ để không phá bookmark/API. | `12 passed`; `py_compile`, `node --check` và `git diff --check` đạt. | Sau khi nghiệm thu UI, tiếp tục TTL/reconciliation của BS-02. |
