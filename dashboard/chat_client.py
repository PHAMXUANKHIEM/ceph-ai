...
def _tool_schemas(*, is_admin: bool = False, cluster=None) -> list[dict]:
    ...
    tools.extend([
        *fixed_query_tools,
        ...
        _fn(TOOL_GET_DISK_FAILURE_RISK, "Risk score ổ đĩa/OSD từ DeviceHealth, latency, I/O và restart evidence đã lưu."),
        _fn(
            TOOL_GET_RBD_TRASH,
...
def _run_tool(name: str, args: dict, actor: str | None = None, cluster=None) -> tuple[str, bool]:
    ...
    try:
        ...
        elif name == TOOL_GET_DISK_FAILURE_RISK:
            result_text, is_error = json.dumps(
                disk_failure_prediction(_cluster_id(cluster)), ensure_ascii=False
            ), False
