...
async def deploy_cluster_provision_host_key(request: Request, user: str = Depends(require_login)):
    """Replace a node SSH pin only with an operator-verified public key."""
    if not auth.is_admin_user(user):
        raise HTTPException(
            status_code=403,
            detail="Chỉ tài khoản admin mới được phép cập nhật SSH host key",
        )
    ...
    return JSONResponse(
        {
            ...
            "message": (
                ...
                "Có thể chạy lại cài đặt; hệ thống vẫn từ chối key chưa được provision."
            ),
        }
    )
...
async def deploy_cluster_forget_host_key(request: Request, user: str = Depends(require_login)):
    """Emergency removal of one pin; provision a verified replacement before retrying."""
    if not auth.is_admin_user(user):
        raise HTTPException(
            status_code=403,
    ...
    if removed:
        return JSONResponse(
            {
                "success": True,
                "message": f"Đã xoá SSH host key cũ của {host}. Hãy provision SSH host key đã xác minh trước khi chạy lại.",
            }
        )
    return JSONResponse(
        {
