(function () {
  ...
  var pageSize = document.getElementById("block-storage-page-size");
  if (!table) return;
  ...
  if (pageSize) pageSize.addEventListener("change", function () {
    var url = new URL(window.location.href); url.searchParams.set("page", "1"); url.searchParams.set("page_size", pageSize.value); window.location.href = url.toString();
  });
