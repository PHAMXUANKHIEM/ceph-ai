(function () {
  ...
  function renderRows(data) {
    ...
    data.items.forEach(function (item) {
      ...
      cell(row, String(item.snapshot_count || 0), "num"); var status = document.createElement("span"), active = Number(item.snapshot_count || 0) > 0; status.className = "volume-status " + (active ? "active" : "idle"); status.innerHTML = '<i class="volume-status-dot"></i>' + (active ? "Active" : "Idle"); cell(row, status); cell(row, "›", "volume-row-chevron"); tbody.appendChild(row);
    });
