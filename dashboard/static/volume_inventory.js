(function () {
  ...
  var metricHealthStatus = document.getElementById("volume-metric-health-status"), metricHealthDetails = document.getElementById("volume-metric-health-details");
  var aiDiagnose = document.getElementById("volume-ai-diagnose"), aiDiagnosisResult = document.getElementById("volume-ai-diagnosis-result");
  ...
  function policyField(name) { return policyHealth && policyHealth.querySelector('[data-policy-field="' + name + '"]'); }
  function setPolicyField(name, value) { var target = policyField(name); if (target) target.textContent = value; }
  function loadPolicyHealth() {
    if (!policyHealth) return;
    requestJson("/api/volumes/" + encodeURIComponent(pool) + "/durability-policy").then(function (data) {
      var policy = data.policy || {}, crush = data.crush || {}, gaps = (data.evidence || {}).gaps || [], notes = data.notes || [];
      var status = policyField("status");
      if (status) { status.textContent = data.status || "INSUFFICIENT_EVIDENCE"; status.className = "volume-capacity-status status-" + String(data.status || "insufficient").toLowerCase(); }
      if (policy.mode === "replicated") { setPolicyField("policy", "Replica x" + (policy.size == null ? "?" : policy.size)); setPolicyField("policy-detail", "min_size " + (policy.min_size == null ? "?" : policy.min_size)); }
      else if (policy.mode === "erasure_coded") { setPolicyField("policy", "EC " + (policy.k == null ? "?" : policy.k) + "+" + (policy.m == null ? "?" : policy.m)); setPolicyField("policy-detail", policy.profile || "profile chưa rõ"); }
      else { setPolicyField("policy", "Chưa rõ"); setPolicyField("policy-detail", "Không xác định được mode"); }
      setPolicyField("rule", crush.rule_name || String(crush.rule_id || "Chưa rõ"));
      setPolicyField("rule-detail", crush.failure_domain ? "chooseleaf " + crush.failure_domain : "Failure domain chưa rõ");
      setPolicyField("domains", String(crush.available_domains || 0) + " / " + String(crush.required_domains || 0));
      setPolicyField("domains-detail", crush.failure_domain ? crush.failure_domain + " domains" : "Topology chưa đủ evidence");
      setPolicyField("migration", data.migration_required ? "Cần plan" : "Không tự động");
      setPolicyField("migration-detail", data.policy_change_supported ? "Có capability" : "Chỉ review/read-only");
      var details = policyField("details");
  ...
  function loadMetricHealth() { if (!metricHealthStatus || !metricHealthDetails) return; requestJson("/api/volumes/" + encodeURIComponent(pool) + "/metrics/summary?hours=24").then(function (data) { var summary = data.summary || {}, consumers = summary.top_consumers || [], status = summary.stale ? "STALE" : "FRESH"; metricHealthStatus.textContent = status; metricHealthDetails.innerHTML = ""; var headline = document.createElement("p"); headline.textContent = "IOPS " + Number(summary.iops || 0).toFixed(1) + " · p95 " + (summary.latency_p95_ms == null ? "—" : Number(summary.latency_p95_ms).toFixed(2) + " ms") + " · freshness " + (summary.freshness_seconds == null ? "—" : Number(summary.freshness_seconds).toFixed(0) + " s"); metricHealthDetails.appendChild(headline); var list = document.createElement("ul"); consumers.slice(0, 5).forEach(function (item) { var row = document.createElement("li"); row.textContent = item.image + " · " + Number(item.iops || 0).toFixed(1) + " IOPS · " + (item.share_of_pool_iops == null ? "—" : (Number(item.share_of_pool_iops) * 100).toFixed(1) + "% pool"); list.appendChild(row); }); metricHealthDetails.appendChild(list); var events = data.correlation_events || []; if (events.length) { var correlation = document.createElement("small"); correlation.textContent = "Lifecycle events gần đây: " + events.map(function (item) { return item.action_id || item.type; }).join(" · "); metricHealthDetails.appendChild(correlation); } }).catch(function (exc) { metricHealthStatus.textContent = "Không đọc được"; metricHealthDetails.textContent = exc.message; }); }
  function runAiDiagnosis() { if (!aiDiagnose || !aiDiagnosisResult) return; aiDiagnose.disabled = true; aiDiagnosisResult.textContent = "Đang phân tích evidence…"; requestJson("/api/volumes/" + encodeURIComponent(pool) + "/inventory-insights/diagnose", { method: "POST", headers: { "Accept": "application/json" } }).then(function (data) { var diagnosis = data.diagnosis || {}; aiDiagnosisResult.textContent = (diagnosis.verdict || "—") + " · " + (diagnosis.severity || "—") + " · " + (diagnosis.summary_vi || "Không có kết luận") + " Bước tiếp theo: " + (diagnosis.recommended_next_step_vi || "—") + " (confidence " + Math.round(Number(diagnosis.confidence || 0) * 100) + "%)"; }).catch(function (exc) { aiDiagnosisResult.textContent = "AI diagnosis chưa khả dụng: " + exc.message; }).finally(function () { aiDiagnose.disabled = false; }); }
  ...
  if (aiDiagnose) aiDiagnose.addEventListener("click", runAiDiagnosis);
  loadOverview(); loadCapacityRisk(); loadDependencyHealth(); loadPolicyHealth(); loadPoolLifecycle(); loadInventory(); loadPoolCounts(); loadDependencyInsights(); loadProtectionInsights(); loadMetricHealth();
}());
