(() => {
  ...
  const batchActions = document.querySelector(".trash-batch-actions");
  ...
  function updateBatchState() {
    ...
    if (batchActions) batchActions.hidden = selected === 0;
    if (selectAll) {
  ...
  async function restoreBatch(rows) {
    ...
    for (const row of rows) {
      ...
      const cluster = new URLSearchParams(window.location.search).get("cluster")
        || form.dataset.cluster
        || document.querySelector(".trash-summary-bar")?.dataset.trashCluster;
  ...
  if (!table || !filterInput || !resetButton || !result || !empty || !pagination || !previousButton || !nextButton || !pageStatus || !pageButtons || !pageSummary) return;
