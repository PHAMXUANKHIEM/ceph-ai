...
def test_pg_filter_script_builds_pool_options_from_rendered_rows():
    ...
    assert "names.forEach((name) => poolSelect.add(new Option(name, name)))" in script
...
def test_pgs_page_returns_all_pgs_with_pool_and_scrub_details(dashboard_client, monkeypatch):
    ...
    assert 'id="pg-bootstrap-data"' in response.text
    ...
    assert 'id="pg-page-size"' in response.text
    assert 'value="50" selected' in response.text
    assert 'id="pg-page-input"' in response.text
    assert 'id="pg-detail-panel"' in response.text
    assert 'data-pg-state="degraded"' in response.text
...
def test_pg_summary_groups_states_pools_and_osd_replica_average():
    summary = pgs_route._pg_summary([
        {"state": "active+clean", "pool": "vms", "acting": [0, 1, 2]},
        {"state": "active+degraded", "pool": "vms", "acting": [0, 1, 2]},
        {"state": "stale+undersized", "pool": "images", "acting": [1, 2]},
    ])

    assert summary["total"] == 3
    assert summary["quick_filters"] == {
        "active+clean": 1, "degraded": 1, "stale": 1, "undersized": 1,
    }
    assert summary["pool_distribution"] == [{"pool": "vms", "count": 2}, {"pool": "images", "count": 1}]
    assert summary["osd_count"] == 3
    assert summary["pgs_per_osd"] == 2.7
